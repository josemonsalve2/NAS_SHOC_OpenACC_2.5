...
#pragma omp target data map  (always tofrom: rsd, frct, u, rho_i, qs, flux_G, utmp_G, rtmp_G)
{  

#ifndef CRPL_COMP
    #pragma omp target parallel for private(k, j, i, m, tmp)
#elif CRPL_COMP == 0
    #pragma omp target parallel for private(k, j, i, m, tmp)
#endif
    for (k = 0; k < nz; k++) {
    ...
    //---------------------------------------------------------------------
#ifndef CRPL_COMP
    #pragma omp target parallel for private (u21, q, k, j, i)
#elif CRPL_COMP == 0
    #pragma omp target parallel for private (u21, q, k, j, i)
#endif
    for (k = 1; k < nz - 1; k++) {
    ...
    if(((jend-jst+1)/32)<32)
      ...
      num_workers2 = 32;
#ifndef CRPL_COMP
    #pragma omp target parallel for private(k, j, m)
#elif CRPL_COMP == 0
    #pragma omp target parallel for private(k, j, m)
#endif
    for (k = 1; k < nz - 1; k++) {
...
#ifndef CRPL_COMP
    #pragma omp target parallel for private(k, i, j, u31, q)
#elif CRPL_COMP == 0
    #pragma omp target parallel for private(k, i, j, u31, q)
#endif
    for (k = 1; k < nz - 1; k++) {
...
#ifndef CRPL_COMP
    #pragma omp target parallel for private(k, i, j)
#elif CRPL_COMP == 0
    #pragma omp target parallel for private(k, i, j)
#endif
    for (k = 1; k < nz - 1; k++) {
...
#ifndef CRPL_COMP
    #pragma omp target parallel for private(k, i, m)
#elif CRPL_COMP == 0
    #pragma omp target parallel for private(k, i, m) 
#endif
    for (k = 1; k < nz - 1; k++) {
...
#ifndef CRPL_COMP
    #pragma omp target parallel for private(k, j, i, m)
#elif CRPL_COMP == 0
    #pragma omp target parallel for private(k, j, i, m)
#endif
    for (k = 1; k < nz - 1; k++) {
    ...
    //---------------------------------------------------------------------
#ifndef CRPL_COMP
    #pragma omp target parallel for private(j, i, k)
#elif CRPL_COMP == 0
    #pragma omp target parallel for private(j, i, k)
#endif
    for (j = jst; j <= jend; j++) {
...
#ifndef CRPL_COMP
    #pragma omp target parallel for private(j, i, k, u41, q)
#elif CRPL_COMP == 0
    #pragma omp target parallel for private(j, i, k, u41, q)
#endif
    for (j = jst; j <= jend; j++) {
    ...
    //---------------------------------------------------------------------
#ifndef CRPL_COMP
    #pragma omp target parallel for private(j, i, m)
#elif CRPL_COMP == 0
    #pragma omp target parallel for private(j, i, m)
#endif
    for (j = jst; j <= jend; j++) {
